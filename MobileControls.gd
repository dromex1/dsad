...
func _update_move_actions(vector: Vector2):
	var threshold := 0.22
...
func _set_action(short_name: String, pressed: bool):
...
func _press_action(action_name: String):
	var input_event := InputEventAction.new()
	input_event.action = action_name
...
func _release_action(action_name: String):
	var input_event := InputEventAction.new()
	input_event.action = action_name
...
func _tap_mouse(button_index: int):
...
func _apply_look(relative: Vector2):
	var mounted_vehicle := _get_mounted_vehicle()
	if mounted_vehicle and mounted_vehicle.has_method("mobile_look"):
		mounted_vehicle.mobile_look(relative)
		return
	var player := get_tree().get_first_node_in_group("player")
	if player and player.has_method("set_rotation_target") and player.is_physics_processing():
		player.set_rotation_target(relative)


func _get_mounted_vehicle():
	for vehicle in get_tree().get_nodes_in_group("vehicle"):
		if vehicle.get("is_mounted") == true:
			return vehicle
	return null
...
func _is_pointer_over_button(event: InputEvent) -> bool:
	...
	if position == Vector2.INF:
		return false
	for button in [action_button, start_button, use_button, pause_button]:
		if button and button.get_global_rect().has_point(position):
			return true
	return false
